...
if (capslock_held || fn2_rec_select_held) {
    ...
    if (k530_bluetooth_is_enabled()) {
        ...
        // LED 61 — батарея (A7[7], 0..100). Всегда показывает заряд (подтверждение
        // сброса перенесено на светодиод состояния 62, красным).
        if (K530_BATT_LED >= led_min && K530_BATT_LED < led_max) {
            uint8_t batt = k530_bluetooth_battery();   // 0..100
            if      (batt > 60u) rgb_matrix_set_color(K530_BATT_LED, 0, 255, 0);    // >60% — зелёный
            else if (batt > 25u) rgb_matrix_set_color(K530_BATT_LED, 255, 120, 0);  // 25..60% — жёлтый
            else                 rgb_matrix_set_color(K530_BATT_LED, 255, 0, 0);    // <25% — красный
        }
    } else {
        // Тумблер BT в OFF — оба индикатора потушены.
        if (K530_HOST_LED >= led_min && K530_HOST_LED < led_max) rgb_matrix_set_color(K530_HOST_LED, 0, 0, 0);
        if (K530_BATT_LED >= led_min && K530_BATT_LED < led_max) rgb_matrix_set_color(K530_BATT_LED, 0, 0, 0);
    }
}